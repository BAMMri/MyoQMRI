...
def main():
    ...
    filename = [x for x in all_files if x.endswith('.dat')]
    if filename == []:
    ...
    kdata = read_raw_data(path+filename)

    # Load associated sequence file
    seq = Sequence()
    seq.read(path + seq_filename, detect_rf_use=True)
    ...
    # get header
    twix = tx.read_twix(path + filename)
    hdr = twix[-1]['hdr']
    ...
    newjsonname = re.search('(.+?)[.]dat', filename).group(1)

    with open(path + newjsonname + '.json', 'w') as f:
