...
def main():
    ...
    path = args.path
    eno1 = args.echonumbers[0]
    eno2 = args.echonumbers[1]
    ...
    # load json file and get relevant parameters
        with open(path + jsonfilename[i]) as json_file:
    ...
    # export as nifti
        try:
            patname = re.search('(.+?)megre', magnnames[i]).group(1)
        except AttributeError:
            patname = '' 

        new_dir = pathlib.Path(path, 'quant')
        new_dir.mkdir(parents=True, exist_ok=True)
        filename_water = patname + 'water.nii.gz'
        filename_fat = patname + 'fat.nii.gz'
        nii_image = nib.Nifti1Image(watermagn, affine=affinematrix)
        nib.save(nii_image, path + '/' + new_dir.name + '/' + filename_water)
        nii_image = nib.Nifti1Image(fatmagn, affine=affinematrix)
        nib.save(nii_image, path + '/' + new_dir.name + '/' + filename_fat)
        if args.ph == True:
            filename_water_phase = patname + 'water_phase.nii.gz'
            filename_fat_phase = patname + 'fat_phase.nii.gz'
            nii_image = nib.Nifti1Image(waterphase, affine=affinematrix)
            nib.save(nii_image, path + '/' + new_dir.name + '/' + filename_water_phase)
            nii_image = nib.Nifti1Image(fatphase, affine=affinematrix)
            nib.save(nii_image, path + '/' + new_dir.name + '/' + filename_fat_phase)
